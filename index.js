...
function parseVersion(raw) {
  ...
  const version = valid(raw) ? raw : coerce(raw);
...
function findAdditionalManifests(root, manifestPaths) {
  ...
  let packageJSONFiles = fg.sync(manifestPaths, { cwd: root });

  let manifests = packageJSONFiles.map((file) => {
    let absolutePath = join(root, file);
    let pkgInfo = JSONFile.for(absolutePath);

    let relativeRoot = dirname(file);
...
class JSONFile {
  ...
  write() {
    ...
    writeFileSync(this.filename, contents + this.trailingWhitespace, { encoding: 'utf8' });
  }
}

export default class WorkspacesPlugin extends Plugin {
  static isEnabled(options) {
    return existsSync(ROOT_MANIFEST_PATH) && options !== false;
  }
  ...
  constructor(...args) {
    ...
    this.registerPrompts({
      publish: {
        type: 'confirm',
        message: (context) => {
          const { distTag, packagesToPublish } = context['release-it-workspaces'];
      ...
      'publish-as-public': {
        type: 'confirm',
        message(context) {
          const { currentPackage } = context['release-it-workspaces'];
  ...
  getWorkspaces() {
    ...
    let packageJSONFiles = fg.sync(
      workspaces.map((glob) => `${glob}/package.json`),
      { cwd: root }
    );
