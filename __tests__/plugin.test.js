...
const namespace = 'release-it-workspaces';
...
function buildPlugin(cwd = process.cwd(), config = {}, _Plugin = TestPlugin) {
  const container = {};
...
describe('release-it-workspaces', () => {
  let dir;
  ...
  describe('normal project setup', () => {
    ...
    it('works', async () => {
      let plugin = buildPlugin(dir.path());
    ...
    it('can specify custom workspaces (overrides package.json settings)', async () => {
      ...
      let plugin = buildPlugin(dir.path(), { workspaces: ['dist/packages/*'] });
    ...
    it('uses specified distTag', async () => {
      let plugin = buildPlugin(dir.path(), { distTag: 'foo' });
    ...
    it('skips registry checks with skipChecks', async () => {
      let plugin = buildPlugin(dir.path(), { skipChecks: true });
  ...
  describe('getWorkspaces', () => {
    function workspaceInfoFor(name) {
      ...
      return {
        ...
        root: dir.path(`packages/${name}`),
        relativeRoot: `packages/${name}`,
        pkgInfo: {
          indent: 2,
          lineEndings: '\n',
          trailingWhitespace: '',
          filename: dir.path(`packages/${name}/package.json`),
          pkg,
        },
      };
    }
